"""
GPyTorch SGPR (Sparse GP Regression) baseline for hyperparameter learning.

Uses the Titsias (2009) collapsed bound via ExactGP + InducingPointKernel.
"""
...
def _require_gpytorch() -> None:
    if gpytorch is None:
        raise ImportError(
            "utils.sgpr requires gpytorch in the active environment."
...
class _SGPRModel(gpytorch.models.ExactGP):
    def __init__(
        ...
        self.scale_kernel = gpytorch.kernels.ScaleKernel(self.base_kernel)
        # InducingPointKernel must be outermost for GPyTorch SGPR prediction
...
def fit_sgpr(
    ...
    *,
    ...
    """
    Fit a GPyTorch SGPR model (Titsias 2009) and return training logs.

    Uses ExactGP with InducingPointKernel and ExactMarginalLogLikelihood.
    """
    _require_gpytorch()
    ...
    with torch.no_grad():
        ...
        if init_outputscale is not None:
            model.scale_kernel.outputscale = float(init_outputscale)
    ...
    # model.parameters() already includes likelihood params via InducingPointKernel
    ...
    history["outputscale"].append(float(model.scale_kernel.outputscale.detach().item()))
    ...
    for iteration in range(max_iters):
        ...
        os_val = float(model.scale_kernel.outputscale.detach().item())
        ...
        if verbose:
            print(
                f"[SGPR M={m}] iter {iteration + 1:>3}/{max_iters}  "
